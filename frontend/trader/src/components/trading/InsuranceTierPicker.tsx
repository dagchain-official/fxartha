...
export default function InsuranceTierPicker(props: Props) {
  ...
  return (
    <div className="rounded-xl border border-border-primary bg-card-nested p-3 space-y-3">
      ...
      >
        <div
          className={`w-9 h-5 rounded-full relative transition-colors ${enabled ? 'bg-[#ccff00]' : 'bg-bg-hover'}`}
        >
          <span
            ...
            {DURATION_OPTIONS.map((opt) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: active ? 'rgba(204,255,0,0.14)' : 'var(--bg-card)',
                    border: `1px solid ${active ? '#ccff00' : 'var(--border-primary)'}`,
                    color: active ? '#ccff00' : 'var(--text-secondary)',
                  }}
                >
                  {opt.label}
                </button>
              ...
              {quotes.map((q) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: active ? 'rgba(204,255,0,0.10)' : 'var(--bg-card)',
                      border: `1px solid ${active ? '#ccff00' : 'var(--border-primary)'}`,
                      boxShadow: active ? '0 0 0 2px rgba(204,255,0,0.2)' : 'none',
                    }}
                  >
                    ...
                    <p className="text-[10px] text-[#ccff00] mt-0.5 font-semibold">

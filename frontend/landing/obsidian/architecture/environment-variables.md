...
| `NEXT_PUBLIC_TRADE_URL` | public | Origin of the FX Artha **trader platform** (fxartha monorepo `frontend/trader`) that all account CTAs link into (`<url>/auth/login`, `<url>/auth/register` via `tradeConfig` in `src/lib/site.ts`). Falls back to `http://localhost:3001` for local dev — **set it in production**. ADR-0022. |

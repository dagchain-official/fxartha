...
| `/trade` | `src/app/trade/page.tsx` | `views/trade/` → `TradeView` |
| `/platform` | `src/app/platform/page.tsx` | `views/platform/` → `PlatformView` |
| `/earn` | `src/app/earn/page.tsx` | `views/earn/` → `EarnView` |
| `/protection` | `src/app/protection/page.tsx` | `views/protection/` → `ProtectionView` |
| `/company` | `src/app/company/page.tsx` | `views/company/` → `CompanyView` |

Routes mirror the five drawer-nav groups (`navGroups` in
`src/data/mocks/site.ts`). Clicking a group in the drawer slides a
full-screen page panel down over the menu (`chrome/nav-page.tsx`) rendering
the same sections from the same data (`src/data/mocks/nav-pages.ts`); the
routed pages remain the canonical URLs — the panel's "Open full page" link
and the footer columns deep-link to `route#section-id` anchors, so section
`id`s on these pages are part of the public URL surface. Sub-page views must
mount `<MarkReady>` (they render without `IntroVideo`) and open with
`<PageHero>` (back link + h1). ADR: [[decisions-log]] ADR-0019.
...
- `src/app/layout.tsx` — the **root layout**. Holds the provider tree
  (`ScrollLayout` → `AdaptiveGrid` / `ReducedMotion` / `Cookie`), the **site
  chrome** (`TubesCursor`, skip link, `Header`, then `{children}`, `SiteFooter`,
  `NavMenu`, `RequestModal` — see [[components/common]] and ADR-0019), loads the
  Space Grotesk + JetBrains Mono fonts ([[design-system]]) and `globals.css`,
  exports `metadata` + `viewport`, and renders the
  JSON-LD script. See [[data-flow]].

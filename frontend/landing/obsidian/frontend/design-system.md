...
## Current theme state — Obsidian & Lime (ADR-0020)

The site runs the **Obsidian & Lime glassmorphism** theme, adopted 2026-08-01
from a design reference. Fixed dark palette — no `prefers-color-scheme`
override; a light mode would be a different brand.
...
- **Tier 1:** obsidian ink ramp (`--raw-color-ink-950/900/800` =
  ...
  corners).
- **Tier 2:** `--background`, `--foreground` (#ebebeb), `--ink`, `--muted`,
  `--subtle`, `--line` (→ glass border), `--surface(-raised)`, `--accent(-from|-to)`
  (lime), `--accent-positive` (emerald — positive/free indicators), `--glass` +
  `--glass-blur` (always used together — every glass fill carries
  `backdrop-blur-glass`), `--glow`.
- **Bindings:** the colours above plus `--blur-glass`, `--shadow-glow`,
  `--font-sans`/`--font-mono`, `--leading-display` (1.1 — the clip floor for
  [[text-engine]]), `--tracking-display` (**-0.05em**, squeezed display
  headings — pair with `font-bold`), `--tracking-label` (0.2em, tracked-out
  mono caps labels), and `--ease-entrance` (`cubic-bezier(0.4, 0, 0.2, 1)`,
  from the reference).
- **`@layer components`:** `body::before/::after` — the fixed 60px grid and
  noise-SVG overlays (anti-flat-dark-mode textures) at `z-index: 1`, between
  the tubes canvas (z-0) and sections (z-10).
...
Two families, both `next/font/google`, loaded in `src/app/layout.tsx`:

- **Space Grotesk** → `--font-space-grotesk` → `--font-sans` (`font-sans`).
  Body at normal weight; display headings are **`font-bold tracking-display`**
  (squeezed −0.05em).
- **JetBrains Mono** → `--font-jetbrains-mono` → `--font-mono` (`font-mono`).
  Technical labels only — eyebrows, table headers, footer column titles, status
  tags — always `uppercase tracking-label` (0.2em).

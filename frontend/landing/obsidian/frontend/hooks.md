...
## `useClock` — `hooks/use-clock.ts`

Live local time + date, ticking once a second. Returns
`{ time: "9:41am", date: "12 March, 2025" }` — those exact fallbacks are the
initial state so the server and client render identical markup; the real values
land on the first tick after mount. Used by the header chip and the nav overlay.

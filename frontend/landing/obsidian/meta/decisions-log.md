...
## ADR-0022 — This site is the landing surface of the fxartha platform

- **Status:** Accepted
- **Date:** 2026-08-03

**Context.** The team's `dagchain-official/fxartha` monorepo is the real
...
group. Two marketing surfaces would drift.

**Decision.**

1. **This site is the single marketing surface**, merged into the monorepo as
   `frontend/landing` (branch `shivam`); the trader app's `(landing)` group
   was deleted and its `/` now `redirect()`s to `/auth/login`.
2. **CTAs funnel into the platform** via `tradeConfig` (`src/lib/site.ts`)
   from `NEXT_PUBLIC_TRADE_URL` (default `http://localhost:3001`):
   login CTAs → `/auth/login`, account/register CTAs → `/auth/register`.
   `SectionText` takes `ctaTarget: "trade" | "modal"` — every conversion CTA
   is `"trade"`; the contact CTA keeps the lead-form modal.
3. **The platform backend is not this repo's concern** — it runs via the
   monorepo's Docker stack; the landing site never talks to it directly.
...
(`~/Desktop/fx-website` and `fxartha/frontend/landing`) — treat fx-website as
the working copy until the branch is merged, then develop in the monorepo.

---

## ADR-0021 — Typewriter/marquee are state-driven content, not text animation

- **Status:** Accepted
- **Date:** 2026-08-01

**Context.** The user asked for a write-and-delete typewriter headline and a
continuously crawling subline ticker. Hard rule #3 forbids TextEngine's
`mode="manual"`, and its declarative modes (`always`/`once`/`forward`/
`progress`) cannot express a timer-driven write→hold→delete→next loop, nor an
endless crawl. Hard rule #1 bans CSS keyframes, the usual marquee/caret tool.

**Decision.** Distinguish two categories:

1. **Glyph animation** — animating the position/opacity/transform of *fixed*
   text → TextEngine, always (unchanged).
2. **State-driven content** — changing *which* text exists over time (a
   typewriter's substring, a counter's number) or translating a laid-out block
   (a marquee row) → plain React state/springs are the right tool, same
   category as the scroll counters. `<Typewriter>` (timer state machine +
   spring-blink caret) and `<Marquee>` (duplicated row + linear-timed spring,
   loop over exactly one copy width) live in `components/ui/`.

Constraints that keep this narrow: no keyframes, caret/crawl timing via
springs; `prefers-reduced-motion` renders the static equivalent; the canonical
copy stays in the DOM for SEO/SR (sr-only text; animated strip aria-hidden).

**Consequences.** Future "typewriter/ticker/counter" asks have a sanctioned
pattern and don't get bent through TextEngine's manual mode. The boundary is
explicit: if the effect moves glyphs of stable text, it is still TextEngine's
job.

---

## ADR-0020 — Adopt the "Obsidian & Lime" glassmorphism theme as a token swap

- **Status:** Accepted
- **Date:** 2026-08-01

**Context.** The user supplied a design reference — obsidian base `#0a0a0a`,
neon-lime `#ccff00` accent, emerald secondary, Space Grotesk + JetBrains Mono,
16px-blur glass surfaces, 60px grid + noise textures, ≥2rem radii — and chose
to implement it directly, retheming the existing layouts rather than rebuilding
them (floating shell / bento grid / split hero deferred).

**Decision.** Execute the retheme at the token layer: swap Tier 1 primitives
and Tier 2 roles in `globals.css` (ADR-0015 makes this the whole point), add
glass tokens (`--glass`, `--glass-blur`, `--line` → rgba-white 10%),
`--shadow-glow`, `--accent-positive`, and type-scale literals
(`--tracking-display` −0.05em, `--tracking-label` 0.2em). Fonts load via
`next/font` with a new `--font-mono` binding. Where the reference conflicts
with project hard rules, the rules win:

- `@keyframes float/pulse` are banned (ADR-0002) — floating cards are skipped
  and status dots are static; anything that later needs real motion uses
  springs.
- Glass fills always pair `bg-glass` with `backdrop-blur-glass` (the
  reference's own legibility requirement, enforced as a convention).
- The grid/noise overlays are `body::before/::after` in `@layer components` —
  the sanctioned pseudo-element case (ADR-0012).

**Consequences.** The visual identity changed with ~zero component API churn;
components that referenced raw surface classes (`bg-ink/85`, `bg-white/[0.04]`)
...
palettes (`TubesCursor`, `AnimatedGradient` `Artha` preset) carry the lime
literals and must be updated together with any future palette change.

---

## ADR-0019 — Multi-page structure: grouped nav routes, chrome in the root layout

- **Status:** Accepted
- **Date:** 2026-08-01

**Context.** The FX Artha site shipped as a one-pager: a ten-item drawer that
...
each needing its own page with a back button — and mounting chrome per-view
would replay the intro film and drop the nav on every sub-page.

**Decision.**

1. **One route per nav group** — `/trade`, `/platform`, `/earn`, `/protection`,
   `/company`, each ADR-0003 compliant (thin route file → `src/views/<route>/`).
   Nav data is `navGroups` (`{label, href, items[]}`) in the site-wide
   `src/data/mocks/site.ts`; drawer entries are `next/link` links (ADR-0005) —
   group label opens the page, sub-labels deep-link to `id` anchors on it.
2. **Chrome lives in the root layout** — `Header`, `NavMenu`, `SiteFooter`,
   `RequestModal` moved to `components/common/chrome/` and mount (with
   `TubesCursor` and the skip link) in `app/layout.tsx`. Views render only
   `<main>` (+ `IntroVideo` on home). One WebGL context, no chrome remounts.
3. **The `ready` gate must be released on every page** — only `IntroVideo`
   flips it, so sub-page views mount `<MarkReady>` (sets `ready` on mount) and
   `IntroVideo` self-skips when the gate is already open. Any future page that
   renders without `IntroVideo` **must** include `<MarkReady>`, or every
   `enabled={ready}` reveal on it stays invisible.
4. **Sub-pages compose generic section components** —
   `components/common/sections/` (`SectionText`, `SectionCards`,
   `SectionTable`, `SectionFaq`, `SectionSteps`), all data-driven client
   leaves. Views stay Server Components that pass serialisable props; CTAs that
   need the modal store go through `OpenAccountButton` rather than passing
   handlers across the RSC boundary.

**Consequences.** New pages are a data file + a view of composed sections +
a three-line route. Chrome copy (nav, footer, modal) lives in `site.ts`, not
`home.ts`. The footer/nav anchor contract is `route#section-id` — section `id`s
on sub-pages are part of the public URL surface and must not be renamed
casually. `sitemap.ts` gains an entry per route.

---

## ADR-0018 — Never animate layout/colour CSS props through a server-rendered spring

- **Status:** Accepted
- **Date:** 2026-07-31

**Context.** The Lumora services rows animate a hover fill — `background-color`
plus a `padding` shift — which was first built with `<Hover>`. It hydrated dirty
on every row. React's SSR pass serialises the spring's `from` object as React
style keys (`paddingLeft`, `backgroundColor: "rgba(241,240,238,0)"`), while
react-spring on the client writes through the CSSOM (`padding-left`,
`background-color: "rgba(241, 240, 238, 0)"`). The values are equivalent; the
strings are not, so React reports a mismatch. Switching bare numbers to `"24px"`
strings fixed only half of it — the key casing and colour normalisation remain.

**Decision.** Springs may animate **transforms and opacity** in server-rendered
markup. Anything else — `padding`, `margin`, `background-color`, `border-color`,
width/height — is either a CSS `transition-*` (ADR-0014 already permits this for
discrete hover/focus state) or lives behind a client-only boundary.

**Consequences.** Hover fills, colour changes and spacing nudges are CSS, which
is what ADR-0014 wanted anyway. The animation system keeps doing what it is good
at. Hydration warnings stop being background noise, so a real one gets noticed.

**Corollary — `lint` and `tsc` do not see this class of bug.** Both passed clean
while the hero image rendered at zero height (a `relative` hardcoded inside
`<LiquidReveal>` losing to the caller's `absolute`) and while every services row
hydrated dirty. Both were found by screenshotting the running page. Drive the
page before calling visual work done.
